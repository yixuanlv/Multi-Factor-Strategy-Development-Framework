...
def main(factor_names=None):
    ...
    try:
        ...
        # 所有文件都保存在 测试结果/多因子共线性分析
        output_dir = os.path.join(work_dir, "../测试结果/多因子共线性分析")
        os.makedirs(output_dir, exist_ok=True)
        image_save_path = os.path.join(output_dir, "多因子共线性分析_分析结果.png")
        ...
        beta_df.to_csv(os.path.join(output_dir, "多因子beta序列.csv"))

        # 保存因子收益率相关性矩阵
        beta_corr = results['beta_corr']
        beta_corr.to_csv(os.path.join(output_dir, "多因子收益率相关性矩阵.csv"))

        # 保存截面因子值相关性均值矩阵
        factor_corr_mean = results['factor_corr_mean']
        factor_corr_mean.to_csv(os.path.join(output_dir, "多因子截面相关性均值矩阵.csv"))

        # 保存累积相关性序列
        cum_corr_cumsum = results['cum_corr_cumsum']
        cum_corr_cumsum.to_csv(os.path.join(output_dir, "多因子累积相关性序列.csv"))

        # 保存原始相关性序列
        cum_corr_df = results['cum_corr_df']
        cum_corr_df.to_csv(os.path.join(output_dir, "多因子相关性序列.csv"))
